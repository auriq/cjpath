#!/bin/bash
###################################################################
# Aggregate CV-PATH pattern
###########################################################################
FROM=$1; TO=$2; DEPTH=$3; FLT=$4; Diff=$5
#FROM=1; TO=10; DEPTH=10; Diff=profile.days

if [ $DEPTH -eq 1 ]; then Path="p1"; DecPath="s:p1"
elif [ $DEPTH -eq 2 ]; then Path="p1 p2"; DecPath="s:p1 s:p2"
elif [ $DEPTH -eq 3 ]; then Path="p1 p2 p3"; DecPath="s:p1 s:p2 s:p3"
elif [ $DEPTH -eq 4 ]; then Path="p1 p2 p3 p4"; DecPath="s:p1 s:p2 s:p3 s:p4"
elif [ $DEPTH -eq 5 ]; then Path="p1 p2 p3 p4 p5"; DecPath="s:p1 s:p2 s:p3 s:p4 s:p5"
...
aq_udb -exp udb_rslt:rslt -sort,dec tcnt rcnt -top $(($TO)) -o,bin - \
	| aq_pp -f,bin - -d s:ik i:tcnt i:rcnt i:tmax i:tmin i:tave -kdec ik $DecPath \
	-eval tave 'tave/tcnt' \
