#!/bin/bash
###################################################################
# Calculate Reverse CV-PATH from first CV
###########################################################################
CreateNode="$1"
Segment="$2"
Grouping="$3"
...
ReDo=$6
#
Hash=$((0x$(sha1sum <<<"$CreateNode $Segment $Grouping")0))
File="rv-path-$Hash.gz"
echo "# Condition=$CreateNode Segment=$Segment Group=$Grouping" > header
#
individual_path () {
# arg1 - udb name, arg2 - filter (option)
eval "aq_udb -ord,dec udb_cj:cj t"  # Reverse to decending order by time
ess exec "aq_udb -exp udb_cj:cj $1 -o,bin - -c uid domain page pname ref ent src skey -local \
	...
	-if -filt 'sPre!=sNow' -eval sPre sNow -o,bin - -c uid ety -endif \
        | aq_pp -f,bin - -d s:uid s:elm -var s:xId '\"\"' -var is:xI2 0 -var s:xS1 '\"\"' \
	 -eval s:p1 '\"\"' -eval s:p2 p1 -eval s:p3 p1 -eval s:p4 p1 -eval s:p5 p1 -eval s:p6 p1 -eval s:p7 p1 \
        ...
        -if -filt 'uid!=xId' -eval xI2 20 -eval xS1 '\"\"' -else -eval xI2 'xI2-1' -endif \
...
eval "aq_udb -ord udb_cj:cj t"  	# Put it back to accending Order
}
#####################
# MAIN 
#####################
...
individual_path "-filt 'fg>0' $Segment"
# output 
if [ $Repo == "local" ]; then
	eval "(cat header ; aq_udb -exp udb_cj:path -filt 'profile.first_cv>0' ) \
        | gzip > $Cache/$File"
else
	ess exec "(cat header ; aq_udb -exp udb_cj:path -filt 'profile.first_cv>0' ) \
